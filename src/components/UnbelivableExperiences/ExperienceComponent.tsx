...
export default function ExperienceComponent(data: ExperienceComponentsProps): ReactElement {
    ...
    return (
        <Paper variant='elevation' sx={{
            ...
            <Box sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'center', alignItems: 'center', columnGap: (theme) => theme.spacing(4) }}>
                <Icon />
                <Typography variant="h4">{data.title}</Typography>
            </Box>
            <Typography color="textSecondary">{data.description}</Typography>
            <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', columnGap: (theme) => theme.spacing() }}>
                <InfoIcon />
